...
const mockSequenceFindMany = vi.fn()

vi.mock('../../../lib/prisma.js', () => ({
  prisma: {
    $queryRaw: (...args: unknown[]) => mockQueryRaw(...args),
    sequence: { findMany: (...args: unknown[]) => mockSequenceFindMany(...args) },
  },
}))

const { nextSequence, formatDocumentNumber, listSequences } = await import('./sequence.service.js')
...
function valuesOf(call: number = 0): unknown[] {
...
describe('nextSequence', () => {
  beforeEach(() => {
    ...
    mockSequenceFindMany.mockReset()
  })
  ...
  it('sérialise deux appels concurrents en deux numéros distincts', async () => {
    ...
    mockQueryRaw.mockResolvedValueOnce([{ next_value: 2 }]).mockResolvedValueOnce([
      { next_value: 3 },
    ])
  ...
  it('utilise année 0 / mois 0 pour un compteur global', async () => {
    mockQueryRaw.mockResolvedValueOnce([{ next_value: 2 }])
    await nextSequence('OPPORTUNITY', new Date(2026, 6, 15), { period: 'GLOBAL' })
    expect(valuesOf()).toEqual(['OPPORTUNITY', 0, 0])
  })
...
describe('formatDocumentNumber', () => {
  it('reproduit exactement le format historique des factures', async () => {
  ...
  it('complète le mois sur deux chiffres', async () => {
    expect(formatDocumentNumber('PCS', new Date(2026, 0, 1), 7)).toBe('PCS-202601-00007')
  })

  it('ne tronque pas au-delà de cinq chiffres', async () => {
    expect(formatDocumentNumber('PCS', new Date(2026, 6, 1), 123456)).toBe('PCS-202607-123456')
  })

  it('accepte d’autres préfixes de document', async () => {
    expect(formatDocumentNumber('BC', new Date(2026, 6, 1), 3)).toBe('BC-202607-00003')
  })
})

describe('listSequences', () => {
  it('trie par clé puis par période décroissante', async () => {
    mockSequenceFindMany.mockResolvedValueOnce([])
    await listSequences()
    expect(mockSequenceFindMany).toHaveBeenCalledWith({
      orderBy: [{ key: 'asc' }, { year: 'desc' }, { month: 'desc' }],
    })
  })
})
