...
<div class="callout">
<p class="lead">État de livraison : <strong>la console ERP existe, elle n'est pas encore en production</strong>.</p>
<p>Le socle <code>erp.pieces.ci</code> (routage du sous-domaine, fiches équipe, capacités, cockpit, tâches, notes, numérotation) est développé et testé sur la branche <code>feat/erp-crm-socle</code>, non fusionnée. Les dix-neuf modules métier, eux, tournent aujourd'hui en production sous <code>pieces.ci/admin</code>. Ce manuel décrit la cible et signale, écran par écran, où l'on en est.</p>
</div>
...
| | Avant | Après |
|---|---|---|
| **Adresse** | `pieces.ci/admin` | `erp.pieces.ci` |
| **Connexion** | Compte Pièces | Le même compte — le cookie est partagé sur tout `.pieces.ci` |
| **Qui entre** | Rôle `ADMIN` | Fiche **équipe** avec un rôle métier (l'`ADMIN` reste admis) |
| **Navigation** | Une liste de 19 entrées | Sections métier, filtrées par capacité |
| **Modules existants** | 19 écrans `/admin/*` | Inchangés et joignables — la console ERP pointe dessus |
...
Sur `erp.pieces.ci`, **tout est réécrit vers la console** — `erp.pieces.ci/taches` sert `/erp/taches`, `erp.pieces.ci/` sert le cockpit. Une courte liste d'exceptions échappe à la règle, parce qu'il faut pouvoir se connecter et lire les mentions légales : `/login`, `/logout`, `/auth`, `/oauth`, `/forgot-password`, `/reset-password`, `/cgu`, `/confidentialite`.

C'est le choix **inverse** de `logistique.pieces.ci`, qui expose une liste blanche de pages. La vitrine logistique est publique et compte une dizaine de pages ; l'ERP n'a aucune page publique et en comptera des dizaines. Maintenir une liste blanche de quarante entrées serait une fabrique à 404 silencieux.

Conséquence pratique : `pieces.ci/erp/…` et `erp.pieces.ci/…` désignent la même chose. Les deux cohabitent — utile en développement, où les sous-domaines locaux sont pénibles, et pour les liens croisés depuis `/admin`.
...
|---|---|---|---|---|
| **Pilotage** | Cockpit | Indicateurs consolidés des trois lignes | `erp:read` | Livré (branche) |
| | Tâches | Travail interne et relances | `erp:read` | Livré (branche) |
...
| **Paramètres** | Équipe | Fiches et habilitations | `erp:read` / `erp:admin` | Livré (branche) |
...
La page d'accueil de la console. Elle **n'agrège que des données existantes** : la phase 1 ne crée aucune donnée métier, elle donne une lecture unique de ce que la plateforme produit déjà.
...
Le carnet de travail commun de l'équipe. Une tâche porte un titre, une description, un responsable, une échéance, une priorité, une ligne d'activité, et — c'est le point important — un **rattachement** à un objet de la plateforme (une commande, un vendeur, une entreprise, un dossier de sourcing).
...
Toute entité de la plateforme peut recevoir des **notes internes** : un commentaire libre, signé et horodaté, visible de l'équipe et jamais du client. Une note peut être **épinglée** — elle remonte alors en tête, ce qui sert pour les consignes permanentes (« Ce client règle toujours à 30 jours, ne pas relancer avant »).
...
| Phase | Contenu | État |
|---|---|---|
| **1 — Socle** | Sous-domaine, fiches équipe, capacités, cockpit, tâches, notes, numérotation | Développée, non fusionnée |
...
### Pour mettre la console en production

1. **Fusionner** la branche `feat/erp-crm-socle` après passage des vérifications (`pnpm lint`, `pnpm build`, `pnpm test`).
2. **Appliquer la migration** de base de données (fiches équipe, tâches, notes, compteurs).
3. **Déclarer `erp.pieces.ci`** en DNS et sur l'hébergement, comme cela a été fait pour `flotte.pieces.ci` et `logistique.pieces.ci`.
4. **Créer les fiches équipe** depuis un compte administrateur : une par membre, avec son vrai métier — pour sortir du régime d'amorçage où tout `ADMIN` peut tout.
5. **Communiquer la nouvelle adresse** à l'équipe et laisser `/admin` en service le temps de la transition.
...
_Document interne Pièces — reflète l'état du code au 4 août 2026 : modules `/admin` en production, socle `erp.pieces.ci` sur la branche `feat/erp-crm-socle`. À réviser après la fusion et après chaque phase._
