...
function fmtFcfa(n: number): string {
  ...
  if (Math.abs(n) >= 1_000)
    return `${(n / 1_000).toLocaleString('fr-FR', { maximumFractionDigits: 0 })} k F`
...
function project(a: Assumptions, scenario: Scenario): MonthRow[] {
  ...
  for (let t = 0; t <= a.horizonMonths; t++) {
    ...
    const newEnterprises =
      Math.max(0, enterprises - prevEnterprises) + (t === 0 ? a.enterprises0 : 0)
  ...
  return (
    <div className="p-4 lg:p-6">
      <header className="mb-6 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h1 className="mt-1 font-display text-2xl text-ink">
            Projections Pièces — revenus, coûts, break-even
          </h1>
          <p className="mt-1 max-w-2xl text-sm text-muted">
            Outil de modélisation interne basé sur des hypothèses paramétrables. Trois sources de
            revenus : commissions marketplace, abonnements Flotte Pro / Flotte Pro +, et logistique
            premium.
          </p>
        </div>
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <Kpi
          label={`MRR à M${a.horizonMonths}`}
          value={fmtFcfa(mrrEnd)}
          hint="Abonnements + premium"
        />
        <Kpi label="ARR projeté" value={fmtFcfa(arrEnd)} hint="MRR × 12" />
        <Kpi
          label="Résultat cumulé"
          value={fmtFcfa(netResult)}
          hint={`Sur ${a.horizonMonths} mois`}
          tone={netResult >= 0 ? 'pos' : 'neg'}
        />
        <Kpi
        ...
        />
        <Kpi
          label={`Entreprises à M${a.horizonMonths}`}
          value={Math.round(lastRow.enterprises).toLocaleString('fr-FR')}
        />
        <Kpi
          label={`Véhicules à M${a.horizonMonths}`}
          value={Math.round(lastRow.vehicles).toLocaleString('fr-FR')}
        />
        <Kpi
          label="Cash mini"
          value={fmtFcfa(minCash)}
          tone={minCash >= 0 ? 'pos' : 'neg'}
          hint="Plancher de trésorerie"
        />
        <Kpi
      ...
      {mixWarning && (
        <div className="mb-4 rounded-sm border border-warn-fg/20 bg-warn-bg px-3 py-2 text-sm text-warn-fg">
          Mix d&apos;adoption ne somme pas à 100 % (actuellement {mixSum.toFixed(0)} %). Les
          projections compensent automatiquement.
        </div>
      )}
      ...
      <div className="grid gap-6 lg:grid-cols-[420px_1fr]">
        ...
        <aside className="space-y-5">
          <Group title="Marché entreprises">
            <NumField
              label="Entreprises au mois 0"
              value={a.enterprises0}
              onChange={(v) => update('enterprises0', v)}
              step={1}
              min={0}
            />
            <NumField
              label="Véhicules moyens / entreprise"
              value={a.avgVehiclesPerEnterprise}
              onChange={(v) => update('avgVehiclesPerEnterprise', v)}
              step={1}
              min={1}
            />
            <PctSlider
              label="Croissance mensuelle entreprises"
              value={a.monthlyEnterpriseGrowthPct}
              onChange={(v) => update('monthlyEnterpriseGrowthPct', v)}
              min={0}
              max={30}
            />
            <PctSlider
          ...
          <Group title="Mix d'adoption (% des entreprises)">
            <PctSlider
              label="Gratuit"
              value={a.mixFreePct}
              onChange={(v) => update('mixFreePct', v)}
              min={0}
              max={100}
            />
            <PctSlider
              label="Flotte Pro · 4 900 F/véh"
              value={a.mixProPct}
              onChange={(v) => update('mixProPct', v)}
              min={0}
              max={100}
            />
            <PctSlider
              label="Flotte Pro + · 9 900 F/véh"
              value={a.mixProPlusPct}
              onChange={(v) => update('mixProPlusPct', v)}
              min={0}
              max={100}
            />
            <PctSlider
              label="% payant à l'année (2 mois offerts)"
              value={a.annualBillingSharePct}
              onChange={(v) => update('annualBillingSharePct', v)}
              min={0}
              max={100}
            />
          </Group>
          ...
          <Group title="Marketplace">
            <NumField
              label="GMV au mois 0 (FCFA)"
              value={a.gmv0}
              onChange={(v) => update('gmv0', v)}
              step={1_000_000}
              min={0}
              format="fcfa"
            />
            <PctSlider
              label="Croissance GMV mensuelle"
              value={a.monthlyGmvGrowthPct}
              onChange={(v) => update('monthlyGmvGrowthPct', v)}
              min={0}
              max={30}
            />
            <PctSlider
            ...
            />
            <NumField
              label="Revenu premium / véhicule Pro+ (FCFA/mois)"
              value={a.premiumRevenuePerPlusVehicle}
              onChange={(v) => update('premiumRevenuePerPlusVehicle', v)}
              step={500}
              min={0}
              format="fcfa"
            />
          ...
          <Group title="Coûts">
            <NumField
              label="Coûts fixes mois 0 (FCFA)"
              value={a.fixedCostsMonth0}
              onChange={(v) => update('fixedCostsMonth0', v)}
              step={500_000}
              min={0}
              format="fcfa"
            />
            <PctSlider
              label="Croissance mensuelle coûts fixes"
              value={a.monthlyFixedGrowthPct}
              onChange={(v) => update('monthlyFixedGrowthPct', v)}
              min={0}
              max={15}
              step={0.5}
            />
            <PctSlider
              label="Coût variable (% du GMV)"
              value={a.variableCostPctOfGmv}
              onChange={(v) => update('variableCostPctOfGmv', v)}
              min={0}
              max={10}
              step={0.1}
            />
            <NumField
              label="CAC / nouvelle entreprise (FCFA)"
              value={a.cacPerEnterprise}
              onChange={(v) => update('cacPerEnterprise', v)}
              step={5_000}
              min={0}
              format="fcfa"
            />
            <NumField
              label="Coût comms / véhicule / mois (FCFA)"
              value={a.commsCostPerVehicle}
              onChange={(v) => update('commsCostPerVehicle', v)}
              step={50}
              min={0}
              format="fcfa"
            />
          ...
          <Group title="Horizon & trésorerie">
            ...
            />
            <NumField
              label="Cash initial (FCFA)"
              value={a.startCash}
              onChange={(v) => update('startCash', v)}
              step={5_000_000}
              min={0}
              format="fcfa"
            />
        ...
        <div className="space-y-5">
                ...
                datasets: [
                  {
                    ...
                    data: rows.map((r) => r.commissions),
                    backgroundColor: '#002366',
                    stack: 'rev',
                  },
                  {
                    label: 'Abonnements Flotte',
                    data: rows.map((r) => r.subscriptions),
                    backgroundColor: '#FF6B00',
                    stack: 'rev',
                  },
                  {
                    label: 'Logistique premium',
                    data: rows.map((r) => r.premium),
                    backgroundColor: '#1E6F4C',
                    stack: 'rev',
                  },
                ],
              }}
              options={{
                ...
                plugins: {
                  ...
                  tooltip: {
                    callbacks: {
                      label: (c: TooltipCtx) =>
                        `${c.dataset.label}: ${fmtFcfaFull(c.parsed.y ?? 0)}`,
                    },
                  },
                },
                scales: {
                  ...
                  y: {
                    stacked: true,
                    beginAtZero: true,
                    ticks: { callback: (v: string | number) => fmtFcfa(Number(v)) },
                  },
                },
              }}
            />
              ...
              data={{
                ...
                datasets: [
                  ...
                  },
                  {
                    ...
                    borderColor: '#1E6F4C',
                    backgroundColor: 'rgba(30,111,76,0)',
                    fill: false,
                    tension: 0.25,
                    borderDash: [4, 4],
                  },
            ...
            <Line
              ...
              options={{
                ...
                plugins: {
                  ...
                  tooltip: {
                    callbacks: { label: (c: TooltipCtx) => fmtFcfaFull(c.parsed.y ?? 0) },
                  },
                },
                scales: { y: { ticks: { callback: (v: string | number) => fmtFcfa(Number(v)) } } },
              }}
                  ...
                  {rows.map((r) => (
                    <tr key={r.month} className="border-t border-border">
                      ...
                      <td className="px-3 py-1.5 text-right">
                        {Math.round(r.enterprises).toLocaleString('fr-FR')}
                      </td>
                      <td className="px-3 py-1.5 text-right">
                        {Math.round(r.vehicles).toLocaleString('fr-FR')}
                      </td>
                      ...
                      <td
                        className={`px-3 py-1.5 text-right ${r.profit >= 0 ? 'text-success-fg' : 'text-error-fg'}`}
                      >
                        {fmtFcfa(r.profit)}
                      </td>
                      <td
                        className={`px-3 py-1.5 text-right ${r.cumCash >= 0 ? 'text-ink' : 'text-error-fg'}`}
                      >
                        {fmtFcfa(r.cumCash)}
                      </td>
                    </tr>
          ...
          <p className="text-xs text-muted">
            Hypothèses : prix Flotte Pro 4 900 F/véh/mois, Flotte Pro + 9 900 F/véh/mois. Paiement
            annuel applique un rabais de 2 mois sur 12 (~16,7 %) au prorata de la part annuelle. Le
            scénario multiplie la croissance entreprises ({SCENARIO_MULT[scenario].growth}×), la
            croissance GMV ({SCENARIO_MULT[scenario].gmv}×) et décale le mix d&apos;adoption vers
            les tiers payants (×{SCENARIO_MULT[scenario].adoption}).
          </p>
        </div>
      </div>
    </div>
...
function NumField({
  ...
  return (
    <label className="block">
      ...
      />
      {format === 'fcfa' && (
        <span className="mt-0.5 block text-[10px] text-muted">{fmtFcfa(value)}</span>
      )}
    </label>
  )
}
...
function exportCsv(rows: MonthRow[]) {
  ...
  for (const r of rows) {
    lines.push(
      [
        r.month,
        Math.round(r.enterprises),
        Math.round(r.vehicles),
        Math.round(r.vehiclesPro),
        Math.round(r.vehiclesPlus),
        Math.round(r.gmv),
        Math.round(r.commissions),
        Math.round(r.subscriptions),
        Math.round(r.premium),
        Math.round(r.revenue),
        Math.round(r.variableCosts),
        Math.round(r.cacCosts),
        Math.round(r.commsCosts),
        Math.round(r.fixedCosts),
        Math.round(r.totalCosts),
        Math.round(r.profit),
        Math.round(r.cumProfit),
        Math.round(r.cumCash),
      ].join(','),
    )
  }
