...
export async function catalogFetch<T = unknown>(
  path: string,
  init?: RequestInit,
...
/**
 * Upload multipart (photo) vers l'API catalogue. On ne fixe PAS Content-Type :
 * le navigateur pose lui-même le boundary multipart/form-data.
 */
export async function catalogUpload<T = unknown>(
  path: string,
  formData: FormData,
): Promise<{ ok: true; data: T } | { ok: false; message: string }> {
  const token = await getToken()
