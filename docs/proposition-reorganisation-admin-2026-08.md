...
_Proposition interne Pièces — établie le 4 août 2026 à partir de l'état du code : dix-neuf modules `/admin` en production, socle `erp.pieces.ci` sur la branche `feat/erp-crm-socle`. Aucun engagement de délai : les lots sont dimensionnés pour être livrés indépendamment._
