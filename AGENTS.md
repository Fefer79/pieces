...
- **ERP Finance** : module `finance` (préfixe `/api/v1/admin/finance`, guard `ADMIN`) — cockpit comptable **100 % lecture seule** : GMV, commissions, frais livraison / main-d’œuvre, panier moyen, escrow bloqué (instantané) et libéré (période), variations vs mois précédent (null si base nulle) ; buckets mensuels (≤ 24) ; agrégation par vendeur (tri commissions desc) ; exports CSV Excel français (BOM UTF-8, séparateur `;`, CRLF) : commandes, commissions par vendeur, mouvements escrow. Même filtre « commandes `COMPLETED` sur `order.createdAt` UTC » que `getAdminOverview`
- **Marketing** : module `marketing` (préfixe `/api/v1/admin/marketing`, guard `ADMIN`) — campagnes WhatsApp en masse (`MarketingCampaign`, statuts BROUILLON / PLANIFIEE / EN_COURS / TERMINEE / ANNULEE) ciblées via les segments CRM calculés (`resolveClientSegmentIds` / `resolveVendorSegmentIds` de `lib/crmSegments.ts`) ou les tags CRM. Aperçu d’audience (total, opt-outs, sans téléphone, échantillon ≤ 10), lancement immédiat ou planifié (`scheduledAt`), refus audience vide (`MARKETING_EMPTY_AUDIENCE`). Envoi asynchrone via le job `MARKETING_CAMPAIGN_SEND` (enqueue `maxAttempts: 1` — jamais de relance automatique, donc jamais de doublon), exclusion des opt-outs `NotificationPreference.whatsapp` (compte lié pour les vendeurs), traçage d’une `CrmInteraction` RELANCE par destinataire
- **Support & SAV** : module `support` (préfixe `/api/v1/admin/support`, guard `ADMIN`) — listes et fiches admin des litiges (`Dispute` : OPEN → UNDER_REVIEW → RESOLVED_BUYER / RESOLVED_SELLER → CLOSED) et des retours (`ReturnOrder`, machine `TRANSITIONS` dupliquée de `return.service.ts` avec horodatage par statut). REFUNDED exige `refundAmount` (`REFUND_AMOUNT_REQUIRED`) et câble `refundEscrow` (module payment) quand l’escrow est HELD ; notifications WhatsApp best-effort (résolution de litige au plaignant, remboursement / rejet de retour au demandeur)
...
- **ERP Finance admin** : pages `app/(auth)/admin/finance/` (onglets Vue d’ensemble / Vendeurs / Exports via `components/finance/finance-tabs.tsx`) ; appels via `lib/finance-api.ts` (`financeFetch` + `downloadFinanceCsv` pour les exports blob), helpers purs dans `lib/finance-utils.ts`
- **Marketing admin** : pages `app/(auth)/admin/marketing/` (onglets Campagnes / Nouvelle campagne via `components/marketing/marketing-tabs.tsx`, fiche campagne `[id]`) ; appels via `lib/marketing-api.ts` (`marketingFetch`), helpers purs dans `lib/marketing-utils.ts`
- **Support & SAV admin** : pages `app/(auth)/admin/support/` (onglets Litiges / Retours via `components/support/support-tabs.tsx`, fiches `litiges/[id]` et `retours/[id]`) ; appels via `lib/support-api.ts` (`supportFetch`), helpers purs dans `lib/support-utils.ts` (dont `NEXT_RETURN_STATUSES`, miroir de la machine à états API)

### Base de données (`packages/shared`)

- PostgreSQL, schéma Prisma dans `prisma/schema.prisma`
- Modèles clés : `User`, `Vendor`, `CatalogItem`, `Order`, `OrderItem`, `Delivery`, `Dispute`, `EscrowTransaction`, `VehicleMake`, `VehicleModel`, `Enterprise`, `Driver`, `Job`, `CrmInteraction`, `CrmTask`, `CrmTag`, `StockLocation`, `StockLevel`, `StockMovement`, `Supplier`, `PurchaseOrder`, `PurchaseOrderItem`, `TeamMemberProfile`, `AgentObjective`, `AgentCommission`, `MarketingCampaign`, etc.
